                   ...
                   int nCmdShow)
{
	...
	if (!music.openFromFile("audio/Space_music.wav")) {
	...
	AVCContactListener *avcListener = new AVCContactListener(bugsGame);

	bugsGame->getGSM()->getPhysics()->getWorld()->SetContactListener(avcListener);
