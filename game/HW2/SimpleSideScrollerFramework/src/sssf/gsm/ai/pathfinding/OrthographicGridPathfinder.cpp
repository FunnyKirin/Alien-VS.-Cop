...
void OrthographicGridPathfinder::updatePath(AnimatedSprite *sprite)
{
	...
	if ((currentPath->size() > 0) && (currentNode != currentPath->end()))
	{
		...
		/**
		if ((int)vY > 0 && (int)vX == 0)
		{
			sprite->setCurrentState(L"MOVE_DOWN");
		}

		else if ((int)vY < 0 && (int)vX == 0)
		{
			sprite->setCurrentState(L"MOVE_UP");
		}

		else if ((int)vX > 0 && (int)vY == 0)
		{
			sprite->setCurrentState(L"MOVE_RIGHT");
		}
		...
		{
			sprite->setCurrentState(L"MOVE_LEFT");
		}
